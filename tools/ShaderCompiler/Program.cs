...
internal static class Program
{
    private static unsafe int Main(string[] args)
    {
        ...
        // The preamble is prepended to every shader, so changing it changes every output even
        // though no .glsl did. Its fingerprint is kept beside the manifest; when it moves, nothing
        // cached is reusable. Without this the tool reports success and leaves every .spv stale.
        string preambleFingerprint = Sha256(
            VulkanGlslPreamble.Apply("#version 450\n", "vert")
            + "\u0000"
            + VulkanGlslPreamble.Apply("#version 450\n", "frag"));
        string preamblePath = Path.Combine(outputDirectory, ".preamble-sha256");
        bool preambleMoved = !File.Exists(preamblePath)
            || !string.Equals(File.ReadAllText(preamblePath).Trim(), preambleFingerprint, StringComparison.Ordinal);
        ...
        IReadOnlyDictionary<(string Name, string Stage), ShaderStageResult> previousStages =
            force || preambleMoved
                ...
                : LoadPreviousStages(manifestPath);
        ...
        var manifest = new ShaderManifest(
            "Regenerate with tools/compile-shaders.ps1.",
        ...
        File.WriteAllText(preamblePath, preambleFingerprint + Environment.NewLine);
