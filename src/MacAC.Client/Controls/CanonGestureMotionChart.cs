...
internal static class CanonGestureMotionChart
{
    private const uint EmoteFeedLookup = 0x10000006u;
    private const uint LeadEmoteAct = 0x10000098u;
    ...
    public static bool TryFetchLocomotion(FeedAct act, out uint locomotion)
    {
        locomotion = 0u;
        if (!CanonActionIdentityTable.TryFetchCanonPersona(
                act,
                out var persona)
            || persona.InputMapId != EmoteFeedLookup)

            return false;

        uint ordinal = persona.ActionId - LeadEmoteAct;
        if (ordinal >= Motions.Length)
            return false;

        locomotion = Motions[ordinal];
        return true;
    }
}
