...
#define sampleTerrain(uvw) MACAC_SAMPLE_ARRAY(uTextureIndexA, uvw)
#define sampleAlpha(uvw)   MACAC_SAMPLE_ARRAY(uTextureIndexB, uvw)

layout(std140, MACAC_UBO_SET binding = 3) uniform TerrainTiling {
    float uTexTiling[36];
};
...
float landTiling(float layer)
{
    return uTexTiling[int(layer)];
}

vec4 bitmaskBlend3(vec4 t0, vec4 t1, vec4 t2, float h0, float h1, float h2)
{
...
vec4 combineTopLayers(vec2 rootST, vec4 pOverlay0, vec4 pOverlay1, vec4 pOverlay2)
{
    ...
    if (h0 > 0.0)
{
        t0 = sampleTerrain(vec3(rootST * landTiling(pOverlay0.z), pOverlay0.z));
        if (pOverlay0.w >= 0.0)
{
    ...
    if (h1 > 0.0)
{
        t1 = sampleTerrain(vec3(rootST * landTiling(pOverlay1.z), pOverlay1.z));
        if (pOverlay1.w >= 0.0)
{
    ...
    if (h2 > 0.0)
{
        t2 = sampleTerrain(vec3(rootST * landTiling(pOverlay2.z), pOverlay2.z));
        if (pOverlay2.w >= 0.0)
{
    ...
    return bitmaskBlend3(t0, t1, t2, h0, h1, h2);
}

vec4 combineRoad(vec2 rootST, vec4 pRoad0, vec4 pRoad1)
{
    ...
    if (h0 > 0.0)
{
        result = sampleTerrain(vec3(rootST * landTiling(pRoad0.z), pRoad0.z));
        if (pRoad0.w >= 0.0)
{
...
void main()
{
    vec4 baseTintVal = vec4(0.0);
    if (vBaseTexIdx >= 0.0)
{
        baseTintVal = sampleTerrain(vec3(vRootST * landTiling(vBaseTexIdx), vBaseTexIdx));
    }

    vec4 overlays = vec4(0.0);
    if (vOverlay0.z >= 0.0)
        overlays = combineTopLayers(vRootST, vOverlay0, vOverlay1, vOverlay2);

    vec4 roads = vec4(0.0);
    if (vRoad0.z >= 0.0)
        roads = combineRoad(vRootST, vRoad0, vRoad1);

    vec3 rootMasked = baseTintVal.rgb * ((1.0 - overlays.a) * (1.0 - roads.a));
    vec3 ovlMasked  = overlays.rgb  * (overlays.a        * (1.0 - roads.a));
    vec3 roadMasked = roads.rgb     *  roads.a;
    vec3 rgb = clamp(rootMasked + ovlMasked + roadMasked, 0.0, 1.0);

    vec3 canvasToLamp = normalize(uShadowLightDirectionAndSource.xyz);
    float directedVis = macacDirectedShadeVis(
        vRealmSpot,
        normalize(vRealmNorm),
        uCameraAndTime.xyz,
        canvasToLamp);
    vec3 lighting = vAmbientOwnLit
        + vDirectedLit * directedVis;
    vec3 shaded = rgb * min(lighting, vec3(1.0));

    float flash = uFogParams.z;
    shaded += flash * vec3(0.6, 0.6, 0.75);

    shaded = enactFog(shaded, vRealmSpot);

    fragTint = vec4(shaded, 1.0);
}
