...
vec3 amassAmbientOwnLamps(
    vec3 N,
    vec3 realmSpot,
    int instOrdinal,
    out vec3 directedLit)
{
    vec3 shaded = uCellAmbient.xyz;
    directedLit = vec3(0.0);
            ...
            for (int i = 0; i < 8; ++i)
{
                ...
                float ndl = max(0.0, dot(N, Ldir));
                directedLit += uLights[i].colorAndIntensity.xyz
...
void main()
{
    ...
    realmSpot.xyz = macacFoliageDisplace(
        realmSpot.xyz,
        model[3].xyz,
        b.flags,
    ...
    vAmbientOwnLit = amassAmbientOwnLamps(
        vNorm,
        vRealmSpot,
        instOrdinal,
        vDirectedLit);

    vReceivesDirectedShade = (uLightingMode == 0
        && instanceIndoor[instOrdinal] == 0u)
        ? 1u
        : 0u;
    vBmpCoord = aBmpCoord;
